...
def download_pending_complaints_by_category(category_name, complaints):
    """Filter and download pending complaints by category (for regular admin)."""
    filtered = [
        c for c in complaints
        if normalize(c[4]) == normalize(category_name) and c[7] == "Pending"
    ]
    if filtered:
        csv_data = convert_to_csv(filtered)
        st.download_button(
            label="📥 Download Pending Complaints Only",
            data=csv_data,
            file_name=f'pending_complaints_{category_name.lower().replace(" ", "_")}.csv',
            key="download_csv_pending_category",
            mime='text/csv'
        )
    else:
        st.info("✅ No pending complaints available for download.")
...
def admin_login():
    ...
    if st.session_state.get("admin_logged_in", False):
        ...
        # --- Superadmin Filters & Download Options ---
        if role == "superadmin":
            st.markdown("### 📥 Download & Filter Options")
        ...
        # --- CSV Download for Regular Admins ---
                # --- CSV Download for Regular Admins ---
        if role != "superadmin":
            ...
            download_pending_complaints_by_category(category_filter, complaints)
